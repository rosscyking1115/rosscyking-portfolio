...
summary: "A region-aware morning-market web app and an audit-ready, evidence-backed brief engine in one. The radar surfaces a market clock, a FRED overnight-risk rail, most-read finance news with AI summaries, and a Taiwan ETF-vs-benchmark attribution tool; the brief engine generates company briefs from SEC EDGAR + FRED where every claim is validated against a stored source span before it ships, with a click-through evidence ledger. Four localised editions (Taiwan, Korea, UK, EU)."
...
Every claim traces back to a primary source, so a reviewer can verify it rather than take it on faith — the opposite of an LLM that asserts confidently and cites nothing.
