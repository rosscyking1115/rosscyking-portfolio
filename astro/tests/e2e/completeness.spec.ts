...
test.describe("completeness — 404 route (finding #1)", () => {
  ...
  test("the build emits 404.html and Vercel is told to serve it", async () => {
    ...
    expect(existsSync(outputPath("static/404.html")), "404.html was not built").toBe(true);
...
test.describe("completeness — featured showcase (finding #5)", () => {
  test("every visible card carries the reveal animation", async ({ page }) => {
    ...
    for (const [i, name] of names.entries()) {
      expect(name, `card ${i} has no entrance animation`).not.toBe("none");
    }

    // The stagger is the point: identical delays would mean the cards all
    // arrive at once, which is not what the FadeIn wrappers did.
